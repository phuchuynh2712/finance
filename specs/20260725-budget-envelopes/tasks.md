...
- [X] T001 Create a new Supabase migration via `supabase migration new budget_envelopes` (this is the first feature-specific migration in this project — `supabase/migrations/` does not yet exist) and copy the DDL from `specs/20260725-budget-envelopes/contracts/schema.sql` (5 tables, RLS policies, `envelope_balances` view) into the generated file, then apply it with `supabase db push` — **PARTIAL**: migration file created at `supabase/migrations/20260726154343_budget_envelopes.sql` with the full DDL. `supabase db push` was NOT run — it requires `SUPABASE_DB_PASSWORD` (the CLI returned "Unauthorized" without it), which is a credential I don't have and shouldn't attempt to obtain/guess. **User action required**: run `supabase db push` (with the DB password set) to apply this migration before T012's sync worker or T056's RLS verification can be tested against the live project.
