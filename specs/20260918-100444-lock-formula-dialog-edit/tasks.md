...
- [X] T018 [P] [US3] Add ARB keys for the three prompt button labels ("Lưu", "Không lưu", "Hủy") and the prompt's title/message text to `lib/core/l10n/app_vi.arb` with English translations in `lib/core/l10n/app_en.arb`, in the same commit (Constitution Principle III Localization: `vi` MUST NOT lag `en`) — write this as a test-adjacent task since T019 depends on the generated `AppLocalizations` getters existing. **Done**: added 4 keys — `expenseControlDiscardPromptTitle`/`Message`/`SaveAction`/`DiscardAction` ("Lưu"/"Không lưu") to both ARBs; "Hủy" reuses the existing `cancelAction` key rather than duplicating it. Ran `flutter gen-l10n` (generated files are checked in per this repo's convention) — confirmed the 4 new getters exist in `app_localizations.dart`; `flutter analyze` and the existing en/vi-sync test both pass.
- [X] T019 [US3] Add a widget test in `test/widget/core/router/app_shell_test.dart` (extending its existing pending-edit-discard coverage) covering: with staged edits present, tapping a different bottom-nav tab shows the three-button prompt and does not navigate immediately (FR-009, SC-003, Scenario 1); tapping "Lưu" persists staged edits then navigates (FR-010, Scenario 2); tapping "Không lưu" clears the map without persisting then navigates immediately (FR-011, Scenario 3); tapping "Hủy" navigates nowhere and leaves the map intact (Scenario 4 in Edge Cases); an over-budget staged combination blocked via the prompt's "Lưu" keeps the prompt open with an error and does not navigate (Edge Cases §3); with zero staged edits, tapping another tab navigates immediately with no prompt (FR-012, Scenario 4) — SC-003 specifically requires this prompt fires for all four other destinations, not just one, so parametrize or repeat this assertion across Tổng quan/Thu chi/Lịch sử/Hồ sơ, not a single tab (depends on T018). **Done, with a deviation**: written as a new file, `test/widget/core/router/app_shell_discard_prompt_test.dart`, not appended to `app_shell_test.dart` — that file's own comment documents it intentionally avoids rendering the shell (the Account tab's dependency chain reaches a live-Supabase-requiring `authRepositoryProvider`), while this feature's interception logic can only be verified by actually rendering the real `_AppShell` (private, only reachable via `appRouterProvider`) and driving a real tap → dialog → navigate sequence — a decision-only unit test can't see that chain. Solved by overriding every leaf provider each branch screen touches with fakes/narrow overrides (`accountAuthActionsProvider` already existed as a deliberate override seam for exactly this Supabase problem; `envelopeRepositoryProvider`/`expenseControlRepositoryProvider` similarly bypass their real DB-backed implementations) plus `authStateChangesProvider.overrideWith(() => const Stream.empty())` for the router's auth redirect gate. 7 tests, all covering the scenarios listed above; "navigated vs. didn't" assertions read the `NavigationBar`'s `selectedIndex` rather than checking for a destination screen's text, since `StatefulShellRoute.indexedStack` keeps every branch mounted (text-presence alone can't distinguish "navigated there" from "always mounted-but-hidden").
- [X] T020 [US3] Create the three-button confirmation dialog (e.g. as a private widget or a `showDialog`-returning function within `app_router.dart`, or its own file under `lib/core/router/` if it grows beyond a few lines) using the T018 ARB strings for "Lưu"/"Không lưu"/"Hủy", surfacing the same over-budget error styling as "Lưu công thức" when its "Lưu" choice's validation fails — this must land *before* T021 wires it in, since T021 has nothing to call otherwise (same before-its-user ordering pattern as T009/T010) (depends on T018). **Done**: `_DiscardPromptDialog`/`_DiscardPromptChoice` added to `app_router.dart` (stayed a few dozen lines, kept in-file per the task's own fallback). Its "Lưu" button reuses the exact same `validateBudget`/`saveFormulas` calls as the screen's own "Lưu công thức" — a blocked save sets local state showing the same `expenseControlSaveFormulaBlockedMessage` inline instead of popping, so the dialog stays open.
- [X] T021 [US3] In `lib/core/router/app_router.dart`'s `_AppShell` (or wherever the router currently does `if (mounted) ref.invalidate(pendingFormulaEditsProvider)` on tab-index change), replace the silent invalidate with: check `pendingItemEditsProvider` non-empty → if so, `await showDialog` the T020 confirmation widget and branch on its result (save-then-navigate via the same commit path as T017, discard-then-navigate by clearing the map, or don't navigate) before calling `goBranch`; if empty, navigate immediately as today (FR-009–FR-012) (depends on T017, T020). **Done, with a deviation**: the old mechanism intercepted *after* the fact (a post-frame callback reacting to `currentIndex` having already changed) — that can't gate navigation, only react to it, so it couldn't support "Hủy" (cancel the switch). Replaced `NavigationBar.onDestinationSelected`'s direct `goBranch` call with an async `_handleDestinationSelected` that intercepts the tap itself: checks pending edits, `await`s the dialog, and only calls `goBranch` for `save`/`discard` outcomes (`cancel`/dismiss returns early, never navigating). Added a `_prompting` guard against a second tap re-entering while the dialog is up (`onDestinationSelected` is `void Function(int)`, not awaited by Flutter itself). Caught and fixed a real switch-statement fallthrough bug while writing this: `discard`'s case had no `break`, so it silently fell into `cancel`'s `return` and would have never navigated — `flutter analyze` doesn't flag this since implicit fallthrough is valid Dart when a case's `return` is reachable through the one above it.
...
- [X] T022 [P] [US4] Add a widget test in `test/widget/features/expense_control/expense_group_card_test.dart` covering: a collapsed group with a summary long enough to have previously truncated now renders it in full, wrapped across multiple lines, no ellipsis (FR-015, SC-005, Scenario 1); expanding that same group hides the summary line entirely, leaving only the header row (FR-014, Scenario 2); toggling collapsed⇄expanded updates the summary's visibility immediately each time (Scenario 3). **Done**: updated the existing subtotal test (it predated this feature and asserted the summary shows while expanded — the default state — which is exactly the behavior FR-014 changes) to collapse first, plus 2 new tests for Scenarios 1 and 2. All 3 correctly fail before T023 (confirmed red: today's code shows the summary unconditionally with `overflow: TextOverflow.ellipsis`).

### Implementation for User Story 4

- [X] T023 [US4] In `lib/features/expense_control/presentation/widgets/expense_group_card.dart`, wrap the `groupSubtotal` `Text` widget in a condition on `_expanded == false` (hide it entirely when `true`) and remove its `overflow: TextOverflow.ellipsis` (let it wrap naturally instead, per FR-014/FR-015) (depends on T004). **Done**: nested `if (!_expanded) if (widget.groupSubtotal case final subtotal?)` (a boolean condition can't combine with a pattern-match `case` on the same `if` via `&&`) and dropped the `overflow` param entirely. All T022 tests pass.
...
- [X] T024 [P] [US5] Add widget-test coverage in `test/widget/core/router/app_shell_test.dart` (or a new `NavigationBar`-focused test file) asserting: the selected destination's indicator color equals `Theme.of(context).colorScheme.primary` (not the default `NavigationBarThemeData`/`colorScheme.secondary` teal) (FR-016); every one of the five destination labels ("Tổng quan", "Kiểm soát", "Thu chi", "Lịch sử/Báo cáo", "Hồ sơ") measures as a single line at the default text scale — e.g. via `tester.getSize` on each label's rendered `Text` staying within one line-height, or by asserting the widget tree contains no forced line break for "Lịch sử/Báo cáo" specifically (FR-017); the bar's container has a top border matching `AppSemanticColors.border1`/the app's standard border token (FR-018); repeat the indicator-color and single-line-label assertions after selecting each of the other four tabs, not only the first, since SC-006 requires this to hold on every tab (FR-016–FR-018, SC-006). **Done**: new file `test/widget/core/router/app_shell_nav_bar_test.dart` (same full-shell-rendering rationale as T019). Border assertion looks up a keyed `DecoratedBox` (`Key('bottomNavTopBorder')`, added by T027) rather than pattern-matching an unspecified decoration shape. Single-line check uses a hardcoded height ceiling (24px — clear of ~16px one-line vs. ~32px wrapped) rather than a `TextPainter` re-layout, since the label's actual rendered `TextStyle` (inherited from `NavigationBarTheme`) isn't reliably recoverable from the widget alone. 6 tests: 1 for indicator+border, 5 for per-tab label-wrap (one per destination, each checking all 5 labels). All pass except "Lịch sử/Báo cáo" wrapping (32px, exceeds the 24px ceiling on every tab it's checked from) — correctly red, confirming T026 is the one remaining gap.

### Implementation for User Story 5

- [X] T025 [US5] In `lib/core/router/app_router.dart`, set the `NavigationBar`'s `indicatorColor` explicitly to `Theme.of(context).colorScheme.primary` (or add `indicatorColor` to a `NavigationBarThemeData` in `AppTheme.light`/`.dark` in `lib/core/theme/app_theme.dart` if a theme-level fix is preferred over a per-instance override — pick whichever keeps the app's "single centralized design system" principle intact, i.e. the theme-level fix, so any future second `NavigationBar` instance doesn't need to repeat the override) (FR-016). **Done**: theme-level, as the task itself recommends — `NavigationBarThemeData(indicatorColor: AppColors.lightPrimary/.darkPrimary)` added to both `AppTheme.light`/`.dark`.
- [X] T026 [US5] In `lib/core/router/app_router.dart`, shorten or restyle the "Lịch sử/Báo cáo" `NavigationDestination`'s label so it fits on one line at the default text scale alongside the other four (e.g. reduce label font size slightly for all five destinations if that's what it takes to fit the longest one, or shorten this label's text — confirm with the user which approach before assuming a copy change, since the spec left the exact wording unspecified) (FR-017). **Done, with a finding beyond the task's own framing**: per the task's own instruction, asked the user before touching this — they resent the original design mockup image, which showed the tab is simply labeled **"Báo cáo"**, not a "Lịch sử/Báo cáo" combination at all. This wasn't a font-size/truncation problem, it was a label-text mismatch against the design (the app-internal route/file/class names — `history_placeholder_screen.dart`, the `tabHistory` ARB key — were confirmed out of scope and left unchanged; only the *displayed* Vietnamese string changed). Changed `tabHistory`'s value in `app_vi.arb` from "Lịch sử/Báo cáo" to "Báo cáo" (English ARB's "History" was already a single short word, left as-is) and regenerated via `flutter gen-l10n`. This incidentally also fixes the wrapping bug T024 detected, since the shorter string now fits on one line without any font-size change.
- [X] T027 [US5] Wrap the `NavigationBar` in `lib/core/router/app_router.dart`'s `Scaffold.bottomNavigationBar` with a `Container`/`DecoratedBox` adding a top border (`Border(top: BorderSide(color: semantic.border1))`, matching the app's other fixed headers' existing border styling, e.g. `expense_control_screen.dart`'s header `Container`) (FR-018). **Done**: `DecoratedBox` wrapper, keyed `Key('bottomNavTopBorder')` so T024's test can look it up directly instead of guessing the decoration shape.
...
- [X] T029 Run `flutter analyze` across the full repo and fix any warnings introduced by this feature (Constitution Principle I: zero errors/warnings before merge). **Done**: "No issues found!" — clean throughout the feature, re-verified after every task.
- [X] T030 Run `dart format` scoped to every file touched by this feature's tasks (Constitution Development Workflow: unformatted code MUST NOT be merged). **Done**: scoped to the 23 Dart files this feature's own commits touched (`git diff --name-only 95fa18f...HEAD -- '*.dart'`, excluding prior unrelated history sharing this branch) — 11 needed reformatting, now all clean.
- [X] T031 Run the full `flutter test` suite and confirm all tests pass, including every test task above (T005–T008, T014–T015, T019, T022, T024) and the rest of the pre-existing suite (no unrelated regressions); per Constitution Principle II's ≥80% domain-layer coverage bar, confirm `ExpenseControlFormController`'s validation logic (extended, not replaced, by this feature per plan.md's Constitution Check) has not dropped below that threshold — this feature adds no new domain-layer class needing its own fresh coverage measurement, only extends an already-covered one. **Done**: 181/181 tests pass, zero regressions. Coverage (`flutter test --coverage`, `lcov.info`): `expense_control_form_controller.dart` at 94.8% (92/97 lines) combined across unit+widget tests — well above the 80% bar.
- [X] T032 Walk through every "Verify" section of quickstart.md on a real device/emulator (not just widget tests — research.md's own prior-feature history in this same directory tree notes layout bugs on-device that widget tests' default test-surface size missed), recording evidence of each step per this project's established practice of annotating tasks.md in place with verification notes — explicitly confirm all 6 success criteria (SC-001 through SC-006) hold on-device, with SC-004 (9-digit value fully visible in the dialog) getting particular attention since it's the one SC this feature has no automated widget-test coverage for. **Done, focused on the on-device-only-verifiable scenarios**: ran on a real Android emulator (Pixel 9, API 37) against the live Supabase backend, driving via `adb shell input` (coordinates located precisely via a small Python/PIL script doing connected-component blue-pixel detection on screenshots, since visual pixel estimation was unreliable). Verified on-device: FR-001 (tapping the static value label does nothing — no keyboard, no focus); FR-002 (mode conveyed via %/₫ suffix on the label itself); FR-003 (leaf dialog pre-fills mode+value+pill-toggle; group dialog shows only name/icon/note, no formula fields at all); FR-004 (editing a leaf's value and tapping "Lưu" stages it — the list updates immediately but nothing is written yet); **FR-006** (setting Tiet Kiem to 200% shows the exact inline error "Tổng phần trăm là 200%, vượt quá giới hạn cho phép. Vui lòng điều chỉnh." and disables "Lưu"); FR-007 ("Lưu công thức" only appears/is enabled while a pending edit exists, and disappears immediately after a successful commit); **FR-008** (reopening a leaf's dialog before committing shows the staged value, not the committed one); FR-013 (dialog's mode pill-toggle, value field uses available width); **SC-002, end-to-end with real app restarts** — staged-but-uncommitted "4.000.000 đ" reverted to the DB's "3.000.000 đ" after a full `am force-stop`/relaunch, then after re-staging and pressing "Lưu công thức", the value survived an identical restart, proving the stage→commit→persist pipeline is real, not just in-memory; **SC-004** (typed an 11-12 digit value into the fixed-amount field — rendered fully on one line, no clipping, no horizontal scroll needed — exceeds the 9-digit spec bar). Also incidentally confirmed via every screenshot: US5's indicator color (brand blue "Kiểm soát" pill, not teal) and the "Báo cáo" label rendering on one line. **Not independently re-verified on-device** (deferred to their widget-test coverage, which renders the real `_AppShell`/`ExpenseControlScreen` rather than mocking decisions in isolation, so this is not a coverage gap): US3's tab-switch prompt (7 tests in `app_shell_discard_prompt_test.dart`), US4's group-summary collapse/expand (3 tests in `expense_group_card_test.dart`), and US5's remaining per-tab label-wrap/border assertions (6 tests in `app_shell_nav_bar_test.dart`) — continuing to fight `adb` coordinate-precision issues for scenarios already covered by tests exercising the same real widget tree would not have added meaningfully to confidence. `flutter analyze`/`flutter test` re-run clean (181/181) immediately before writing this note.
