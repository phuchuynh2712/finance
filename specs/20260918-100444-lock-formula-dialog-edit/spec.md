# Feature Specification: Align Expense Control Screen with Design (Formula Editing, Group Summary, Bottom Nav)
...
- **FR-001**: The allocation-value input field shown inline in the Expense Control item list MUST be replaced with a plain static label that displays the current value; it MUST NOT accept keyboard focus or text input, and tapping it MUST produce no visible or interactive reaction of any kind (no keyboard, no cursor, no dialog, no popup). This label MUST size itself to its content (not a fixed width) so that a short percentage value (e.g. "24%") and a long fixed-amount value (e.g. "4.000.000 ₫") are both always fully visible, matching the design mockup's own display of both a short and a long value with neither clipped.
...
- User Story 4's collapsed-only visibility for a group's allocation summary is a deliberate deviation from the design mockups, which show the summary present even while a group is expanded — confirmed intentional (to avoid duplicating information already visible per-child) after re-checking the mockups against this decision mid-review.
